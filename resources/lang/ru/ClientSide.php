<?php

return [

    //cinema.blade.php + concert.blade.php + CinemaItem.blade.php + EventItem.blade.php
    'share' => 'Поделиться',
    'buy_ticket' => 'Купить билет',
    'views' => 'Виды',
    'prices_from' => 'Цена от',
    'starting' => 'В кино с',

    //subCategoryList.blade.php
    'date' => 'Дата',

    //FilterMenu.blade.php
    'select' => 'Выберите дату',
    'popular' => 'Популярное',
    'new' => 'Новые',
    'filter' => 'Фильтры',
    'today' => 'Сегодня',
    'tomorrow' => 'Завтра',
    'week' => 'На этой неделе',
    'month' => 'В этом месяце',

    //HomeCinema.blade.php
    'view' => 'Посмотреть все',

    //ItemList.blade.php
    'home' => 'Главная',
    'for_cinema' => 'Билеты на кинотеатры',
    'composers' => 'Исполнители',
    'events' => 'Мероприятие',
    'concerts' => 'Концерты',
    'alternatives' => 'Альтернатива',
    'bards' => 'Барды',
    'jazz' => 'Джаз & блюз',
    'others' => 'Другие',
    'movies' => 'Кино',
    'classic' => 'Классическая музыка',
    'music' => 'Музыкальный спектакль',
    'musical' => 'Мюзикл',
    'ballet' => 'Опера и Балет',
    'operetta' => 'Оперетта',
    'pop' => 'Поп и Эстрада',
    'rave' => 'РЕЙВ',
    'rock' => 'Рок',
    'romance' => 'Романсы',
    'onDate' => '13 сентября',
    'onTime' => '19:00, пятница',
    'title' => '"Человек-паук Вдали от дома"',
    'description' => 'Питер Паркер вместе с друзьями отправляется на летние каникулы в Европу. Однако отдохнуть приятелям вряд ли удастся — Питеру придется согласиться помочь Нику Фьюри раскрыть тайну существ, вызывающих стихийные бедствия и разрушения по всему континенту.',
    'shown' => 'Видно на странице',

    //PublicFooter.blade.php
    'want' => 'Хотите всегда быть в курсе актуальных событий?',
    'subscribe' => 'Подписаться',
    'email' => 'Введите ваш e-mail',
    'email_for' => 'Подпишитесь на рассылку и получайте на почту персональную подборку событий вашего города',
    'cabinet' => 'ЛИЧНЫЙ КАБИНЕТ',
    'introduction' => 'ЗНАКОМСТВО С BILETTM.COM',
    'questions' => 'ВОПРОСЫ И ОТВЕТЫ',
    'offices' => 'БИЛЕТНЫЕ ОФИСЫ',
    'rassylka' => 'РАССЫЛКА',
    'collective' => 'КОЛЛЕКТИВНЫЕ ЗАКАЗЫ',
    'organizers' => 'ОРГАНИЗАТОРАМ',
    'concert_halls' => 'КОНЦЕРТНЫМ ПЛОЩАДКАМ',
    'partners' => 'ПАРТНЁРАМ',
    'logo' => 'ЛОГОТИП ДЛЯ АФИШ И СМИ',
    'addEvent' => 'Добавить событие',
    'ticket_service' => 'Билетный сервис',
    'copyright' => 'Все права защищены',

    //PublicHeader.blade.php
    'placeholder' => 'Мероприятия, исполнители, площадки',

    //AddEventForm.blade.php
    'send' => 'Отправить',
    'required' => 'Обязательные поля',
    'name' => 'Имя',
    'phone' => 'Телефон',
    'venue' => 'Место проведения',
    'place' => 'Концертный зал Туркменистан',
    'message' => 'Сообщение',
    'text' => 'Пришлите нам информацию о мероприятии, которое вы хотите организовать! Мы увидим информацию и свяжемся с Вами как можно скорее.',

    //EventsPage.blade.php
    'rep' => 'Весь репертуар',

    //SearchResults.blade.php
    'results' => 'Результаты поиска',

    //SeatsPage.blade.php
    'seats' => 'Выбрать места',
    'available' => 'Доступные',
    'booked' => 'Зарезервировано',
    'reserved' => 'Зарезервировано',
    'selection' => 'Ваш выбор',
    'selected' => 'Вы выбрали',
    'seatsTotal' => 'мест. Общая стоимость',
    'yourSeats' => 'Ваши места',

];